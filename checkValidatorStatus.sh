...
validatorVersionNetwork=`echo $validatorJSON | jq .version | tr -d '"'`
...
versionsCheck="$colorRed❌$colorEnd"
	...
	if [[ -z $validatorVersionNetwork ]]
	then
		validatorVersionNetwork=$gossipVersion
	fi


	if [[ "$validatorVersionLocal" == "$validatorVersionNetwork" ]]
	then
		versionsCheck="$colorGreen✅$colorEnd"
	fi

echo  "Network:     $networkType"
echo  "Version:     $validatorVersionLocal (local) $versionsCheck $validatorVersionNetwork (network)"
...
unixtime=`date +%s`;
echo
echo "$unixtime $epochNumberCurrent $epochProgress $slotsScheduled $slotsBuilt $slotsSkipped $slotsRemaining $balanceValidator $balanceVoteAccount $validatorActivatedStake" >> $logPath/balances-$networkType.log
date
echo
