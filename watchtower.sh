...
source "${scriptPath}/env.sh"
        ...
        if [[ -z $1 ]]; then
                echo "Validator identity unspecified. I gonna use $validatorVoteAccountPubKeyStaked from env.sh instead."
                echo "Usage: $0 <validator-identity-pubkey>"
		validatorVoteAccountPubKey=$validatorVoteAccountPubKeyStaked
        else
...
$execSolanaWatchtower --monitor-active-stake --validator-identity $validatorIdentityPubKey --minimum-validator-identity-balance 3 --interval 30
